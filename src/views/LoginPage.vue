...
export default {
  ...
  methods: {
    ...
    try {
      ...
      const docRef = doc(db, 'medecins', uid);
      ...
      if (docSnap.exists()) {
        ...
        alert("⚠️ Ce compte n'est pas reconnu comme médecin.");
        await auth.signOut(); // pour éviter que le compte reste connecté
      }
    } catch (error) {
      alert("Erreur de connexion médecin : " + error.message);
    }
  },
  ...
  try {
    ...
    // 1️⃣ Vérifie d'abord si c'est un utilisateur classique
    ...
    if (userDoc.exists()) {
      ...
      if (data.type === 'patient') {
        ...
        this.$router.push('/professionnel');
        return;
      } else {
        alert("Type inconnu.");
        return;
    ...
    // 2️⃣ Vérifie si c’est un médecin
    const medecinDoc = await getDoc(doc(db, "medecins", uid));
    if (medecinDoc.exists()) {
      this.$router.push('/medecin');
      return;
    }

    // 3️⃣ Vérifie si c’est une secrétaire
    const secretaireDocRef = doc(db, "secretaires", uid);
    const secretaireDocSnap = await getDoc(secretaireDocRef);

    if (secretaireDocSnap.exists()) {
      const secretaireData = secretaireDocSnap.data();

      if (secretaireData.isOnline) {
        alert("Ce compte est déjà connecté sur un autre appareil.");
        await auth.signOut();
        return;
      }

      // Marquer comme connecté
      await updateDoc(secretaireDocRef, { isOnline: true });
